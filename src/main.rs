...
fn rocket() -> _ {
    rocket::build()
        ...
        .mount("/", routes![events, register, submit_char, lives, game_string, player_number])
        .manage(RwLock::new(GameManager::new()))
        .manage(channel::<String>(1024).0)
}

#[post("/api/register", data = "<username>")]
fn register(cookies: &CookieJar<'_>, game_manager: &State<RwLock<GameManager>>, username: Json<Username<'_>>, event: &State<Sender<String>>) -> (ContentType, String) {
...
fn submit_char(cookies: &CookieJar<'_>, game_manager: &State<RwLock<GameManager>>, character: Json<Character>, event: &State<Sender<String>>) -> (ContentType, String) {
...
#[get("/sse")]
async fn events(event: &State<Sender<String>>, mut end: Shutdown) -> EventStream![] {
